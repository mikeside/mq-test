...
try {
    ...
    foreach ($config as $exchangeType => $item) {
        foreach ($item as $exchange) {
            ...
            // 创建交换机
            $mqChannel->exchange_declare($exchange['exchangeName'], $exchangeType, false, true, false);
            ...
            foreach ($exchange['queue'] as $queue) {
                ...
                // 创建队列
                $mqChannel->queue_declare($queue['queueName'], false, true, false, false);

                // 队列绑定交换机
                $mqChannel->queue_bind($queue['queueName'], $exchange['exchangeName'], $queue['routingKey']);
